...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	if(ret == -1) {
		...
		// find available memory space for elf
		...
		*entry = ((struct ELFHeader *)elf)->entry;
		...
		uint32_t srchwaddr = 0;
		uint32_t desthwaddr = 0;
		for(int i = 0; i < phnum; i++, ph++) {
			if(ph->type == 0x1) { // type == LOAD
				// physAddr is the base address of code segment in this process
				desthwaddr = physAddr + ph->vaddr;
				srchwaddr = elf + ph->off;
				// copy section
				for(int j = 0; j < ph->filesz; j++) {
					*(uint8_t*)(desthwaddr + j) = *(uint8_t*)(srchwaddr + j);
				}
				// clear [vaddr+filesz, vaddr+memsz]
				for(int j = ph->filesz; j < ph->memsz; j++) {
					*(uint8_t*)(desthwaddr + j) = 0;
				}
			}
		}
		return 0;
