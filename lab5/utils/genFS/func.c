...
/*
 *  Clear data blocks used by inode, update superblock availBlockNum & blockBitmap and write back
 */
int clrBlocks(FILE *file, SuperBlock *superBlock, Inode *inode) {
    // TODO in lab5
    int i = 0;
    int j = 0;
    int k = 0;
...
/*
 *  Delete inode, update superblock availInodeNum & inodeBitmap and write back
 */
int clrInode(FILE *file, SuperBlock *superBlock, int inodeOffset) {
    // TODO in lab5
    int j = 0;
    int k = 0;
...
/*
 * Delete all the directory entries in a directory(destInode)
 */
int clrDirEntries (FILE *file, SuperBlock *superBlock, Inode *fatherInode, int fatherInodeOffset, 
                Inode *destInode, int destInodeOffset) {
    // TODO in lab5
    int ret = 0;
    int dirIndex = 0;
    ...
    while (getDirEntry(file, superBlock, destInode, dirIndex, &dirEntry) == 0) {
        dirIndex++;
        ...
        if(childInode.type == REGULAR_TYPE) {
            ret = clrBlocks(file, superBlock, &childInode);
            if(ret == -1)
                return -1;
            ret = clrInode(file, superBlock, childInodeOffset);
        }
        else if(childInode.type == DIRECTORY_TYPE) {
            if(childInodeOffset == fatherInodeOffset || childInodeOffset == destInodeOffset) {
                ...
                ret = clrDirEntries(file, superBlock, destInode, destInodeOffset, &childInode, childInodeOffset);
                if(ret == -1)
                    return -1;
                ret = clrInode(file, superBlock, childInodeOffset);
            }
    ...
    ret = clrBlocks(file, superBlock, destInode);
    if(ret == -1)
        return -1;
    return 0;
}
...
/*
 * Delete the directory entry of destInode from fatherInode
 */
int delDirEntry (FILE *file, SuperBlock *superBlock, Inode *fatherInode, int destInodeOffset) {
    // TODO in lab5
    int i = 0;
    int j = 0;
    int ret = 0;
...
int rmdir (const char *driver, const char *destDirPath) {
    ...
    if (driver == NULL || destDirPath == NULL) {
        printf("driver == NULL || destFilePath == NULL.\n");
        return -1;
    }
    file = fopen(driver, "r+");
    if (file == NULL) {
    ...
    *((char*)destDirPath + size + 1) = tmp;
    ...
    ret = readInode(file, &superBlock, &destInode, &destInodeOffset, destDirPath);
    if (ret == -1) {
        printf("Failed to read destination inode.\n");
        fclose(file);
        return -1;
    }
    ...
    ret = clrDirEntries(file, &superBlock, &fatherInode, fatherInodeOffset, &destInode, destInodeOffset);
    if(ret == -1) {
        printf("Fail to delete directory entries.\n");
        fclose(file);
        return -1;
    }
    ret = clrInode(file, &superBlock, destInodeOffset);
    if (cond == 1)
        *((char*)destDirPath + length - 1) = '/';
    ret = delDirEntry(file, &superBlock, &fatherInode, destInodeOffset);
    if(ret == -1) {
        printf("Failed to delete directory entry.\n");
        fclose(file);
        return -1;
    }
...
int rm (const char *driver, const char *destFilePath) {
    ...
    if (driver == NULL || destFilePath == NULL) {
        printf("driver == NULL || destFilePath == NULL.\n");
        return -1;
    }
    file = fopen(driver, "r+");
    ...
    *((char*)destFilePath + size + 1) = tmp;
    ret = readInode(file, &superBlock, &destInode, &destInodeOffset, destFilePath);
    if (ret == -1) {
        printf("Failed to read destination inode.\n");
        fclose(file);
        return -1;
    ...
    /*
    if(ret == -1) {
        printf("Failed to delete destination inode.\n");
        fclose(file);
        return -1;
    }
    */
