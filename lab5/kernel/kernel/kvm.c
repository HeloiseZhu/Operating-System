...
void initFS () {
	...
	Inode shMemInode;
	Inode devInode;
	int stdInInodeOffset = 0;
	int stdOutInodeOffset = 0;
	int shMemInodeOffset = 0;
	int devInodeOffset = 0;
	...
	for (i = 0; i < MAX_SYSFCB_NUM; i++) {
		sysFcb[i].state = 0;	// 0: not in use, 1: in use
		sysFcb[i].inodeOffset = 0;
		sysFcb[i].linkCount = 0;
		sysFcb[i].offset = 0;
	}
	mkdir(&sBlock, "/dev", &devInode, &devInodeOffset);
	createFile(&sBlock, "/dev/stdin", &stdInInode, &stdInInodeOffset);
	dev[STD_IN].inodeOffset = stdInInodeOffset;
	sysFcb[STD_IN].state = 1;
	sysFcb[STD_IN].linkCount = 1;
	sysFcb[STD_IN].inodeOffset = stdInInodeOffset;
	createFile(&sBlock, "/dev/stdout", &stdOutInode, &stdOutInodeOffset);
	dev[STD_OUT].inodeOffset = stdOutInodeOffset;
	sysFcb[STD_OUT].state = 1;
	sysFcb[STD_OUT].linkCount = 1;
	sysFcb[STD_OUT].inodeOffset = stdOutInodeOffset;
	createFile(&sBlock, "/dev/shmem", &shMemInode, &shMemInodeOffset);
	dev[SH_MEM].inodeOffset = shMemInodeOffset;
	sysFcb[SH_MEM].state = 1;
	sysFcb[SH_MEM].linkCount = 1;
	sysFcb[SH_MEM].inodeOffset = shMemInodeOffset;
	
	for (i = 0; i < MAX_PCB_NUM; i++) {
		for (j = 0; j < MAX_FCB_NUM; j++) {
			pcb[i].fcb[j].state = 0;	// 0: not in use, 1: in use
			pcb[i].fcb[j].index = -1;
			pcb[i].fcb[j].flags = 0;
		}
		pcb[i].fcb[STD_IN].state = 1;
		pcb[i].fcb[j].index = STD_IN;
		sysFcb[STD_IN].linkCount++;
		pcb[i].fcb[STD_OUT].state = 1;
		pcb[i].fcb[j].index = STD_OUT;
		sysFcb[STD_OUT].linkCount++;
		pcb[i].fcb[SH_MEM].state = 1;
		pcb[i].fcb[j].index = SH_MEM;
		sysFcb[SH_MEM].linkCount++;
	}
}
